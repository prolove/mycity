...
class Base extends Model {
    ...
    {
        if(isset($data['twothink_name'])){
            $this->name=$data['twothink_name'];
            unset($data['twothink_name']);
        }
    ...
    public function checkModelAttr($model_id=false,$data=false){
        ...
        //查询模型信息
        if(!$model_id){
            if(!$model_id = Db::name('Model')->where(['name'=>$this->name])->value('id')){
            ...
            switch ($attr['auto_time']){
                ...
                default:
                    if (!empty($attr['auto_rule'])){//始终
                        $auto_data[$attr['name']] = isset($data[$attr['name']]) ? $attr['auto_rule']($data[$attr['name']],$data):'';
