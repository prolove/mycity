...
class Addons extends Admin {
    public function _initialize() {
        $this->assign ( '_extra_menu', array (
            '已装插件后台' => Loader::model('Addons','model',false,'admin')->getAdminList ()
        ) );
        parent::_initialize ();
    }
        ...
        if (isset ( $model )) {
            ...
            if(isset($model->model_info) && !empty($model->model_info)){
                if(is_numeric($model->model_info)){
                    $model_obj = Modelinfo()->modelinfo($model->model_info)->list_field()->fields();
                }else{
    ...
    public function edit($name, $id = 0) {
        ...
        if (isset ( $model )) {
            ...
            if(isset($addonModel->model_info) && !empty($addonModel->model_info)){
                if(is_numeric($addonModel->model_info)){
                    $model_info = Modelinfo()->modelinfo($addonModel->model_info)->list_field()->fields()->FieldDefaultValue()->Getparam('info');
                }else{
                    $model_info = Modelinfo()->info($addonModel->model_info)->FieldDefaultValue()->Getparam('info');
                }
                if(!$model_info['url'])
                    $model_info['url'] = $this->request->url();
                $this->assign( 'model_info', $model_info );
        ...
        if ($this->request->isPost()) {
            ...
            $this->assign ( 'meta_title', $id ? '编辑' . $model_info ['title'] : '新增' . $model_info ['title'] );
